#include "unordered_mapViewer.h"

#include <iostream>



unordered_mapViewer::unordered_mapViewer() {
    std::cout << "hasdasd asadi\n"; 
}

void unordered_mapViewer::create_menu_panel() {
...
void unordered_mapViewer::run() {
    ...
    s_cam_ = std::unique_ptr<pangolin::OpenGlRenderState>(
					new pangolin::OpenGlRenderState(
						pangolin::ProjectionMatrix(unordered_map_viewer_width_, unordered_map_viewer_height_, viewpoint_f_, viewpoint_f_,
															unordered_map_viewer_width_ / 2, unordered_map_viewer_height_ / 2, 0.1, 1e6),
															pangolin::ModelViewLookAt(viewpoint_x_, viewpoint_y_, viewpoint_z_,
															0,0,0, 0.0, -1.0, 0.0)));
    d_cam_ = pangolin::CreateDisplay()
					.SetBounds(0.0, 1.0, pangolin::Attach::Pix(175), 1.0, -unordered_map_viewer_width_ / unordered_map_viewer_height_)
